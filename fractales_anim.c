...
// gcc -O3 fractals_opti.c -pthread $(sdl2-config --cflags --libs) -o fractals_opti

/*
Flags pour SDL_CreateRenderer() :
...
// char *titre(double r, double theta){
//     char *txt = "coucou %d" 5;
//     printf("%s\n", txt);
//     char c_r[5], c_theta[5];
//     snprintf(c_r, 5, "%f", r);
//     snprintf(c_theta, 5, "%f", theta);
//     printf(":%c%c%c: :%s:\n", c_r[0], c_r[1], c_r[2], c_theta);
//     return "Fractals";
// }
...
int main(void){
    ...
    while(running) {
        ...
        while(SDL_PollEvent(&event)) {
            ...
            //    SDL_Log("(%d %d) (%d %d)", event.motion.x, event.motion.y, event.motion.xrel, event.motion.yrel);
            } else if (event.type == SDL_MOUSEBUTTONDOWN){
                //printf("Done\n");
                // SDL_Log("%d", event.wheel.y);
        ...
        if (changes){
            // SDL_SetWindowTitle(window, titre(r, theta));
            if(theta > 180){
                theta -= 360;
            } else if(theta < -180) {
                theta += 360;
